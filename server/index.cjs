...
app.post('/api/expenses', (req, res) => {
  ...
  const info = stmt.run(description, Number(amount), category, date, toMonthKey(date), toYear(date));
...
app.put('/api/expenses/:id', (req, res) => {
  ...
  ).run(newDesc, Number(newAmt), newCat, newDate, toMonthKey(newDate), toYear(newDate), Number(req.params.id));
...
app.post('/api/remittances', (req, res) => {
  ...
  const info = stmt.run(Number(amount), note ?? '', date, toMonthKey(date), toYear(date));
...
app.put('/api/remittances/:id', (req, res) => {
  ...
  db.prepare('UPDATE remittances SET amount=?, note=?, date=?, month=?, year=? WHERE id=?')
    .run(Number(newAmt), newNote, newDate, toMonthKey(newDate), toYear(newDate), Number(req.params.id));
...
app.post('/api/month-config', (req, res) => {
  ...
  `).run(month, Number(miscBudget));
  res.json(db.prepare('SELECT * FROM month_configs WHERE month = ?').get(month));
});

// ─────────────────────────────────────────────────────────────────────────────
// SUMMARY (all data for a month — one API call)
// ─────────────────────────────────────────────────────────────────────────────
...
app.get('/api/backup/json', (req, res) => {
  ...
  res.setHeader('Content-Disposition',
    `attachment; filename="expenseiq-backup-${new Date().toISOString().split('T')[0]}.json"`);
  res.json(payload);
});
...
app.get('/api/backup/csv', (req, res) => {
  ...
  res.setHeader('Content-Disposition',
    `attachment; filename="expenseiq-export-${new Date().toISOString().split('T')[0]}.csv"`);
  res.send(rows.join('\n'));
});
  ...
  const importMany = db.transaction(() => {
    ...
    for (const e of payload.expenses ?? []) {
      ...
      db.prepare(
        ...
        e.month ?? toMonthKey(e.date), e.year ?? toYear(e.date),
    ...
    for (const r of payload.remittances ?? []) {
      ...
      ).run(r.amount, r.note ?? '', r.date,
        r.month ?? toMonthKey(r.date), r.year ?? toYear(r.date),
    ...
    for (const i of payload.investments ?? []) {
      ...
      ).run(i.amount ?? 2500, i.note ?? '', i.date,
        i.month ?? toMonthKey(i.date), i.year ?? toYear(i.date),
...
app.get('/api/health', (req, res) => {
  const counts = {
    ...
    remittances: db.prepare('SELECT COUNT(*) as c FROM remittances').get().c,
    investments: db.prepare('SELECT COUNT(*) as c FROM investments').get().c,
  };
  res.json({ ok: true, db: DB_PATH, time: new Date().toISOString(), counts });
});
...
app.listen(PORT, () => {
  console.log(`\n🚀 ExpenseIQ API server running → http://localhost:${PORT}`);
